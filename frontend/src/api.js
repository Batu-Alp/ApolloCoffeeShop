...
export const updateCash = async (totalCost) => {
  try {
...
export const deliverOrder = async (delivery) => {
  try {
    const { token } = getUserInfo();
    const response = await axios({
      url: `${apiUrl}/api/orders/${delivery._id}/deliver`,
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
